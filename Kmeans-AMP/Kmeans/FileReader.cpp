...
Matrix GetVector(int &x, int &y)
{
	...
	if (myfile.is_open())
	{
		...
		for (int i = 0; i < result.n_col; i++)
		{
			...
			{
				myfile >> result.val[(i*result.n_row) + j];
				if (j == 0)
				{ 
					if (result.val[(i*result.n_row) + j] > x)
						x = result.val[(i*result.n_row) + j];
				}
				else if (j == 1)
				{ 
					if (result.val[(i*result.n_row) + j] > y)
						y = result.val[(i*result.n_row) + j];
				}
			}
		}
		myfile.close();
