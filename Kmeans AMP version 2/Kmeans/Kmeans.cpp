	...
	do
	{
		...
		else
		{
			...
			if (n_Iters >= EST_START)
			{
				...
				if (n_Iters > EST_START)
					for (i = 0; i<col; i++) //sim_Mat[cluster[i]][i]
						sim_Mat[(col*i) + i] = matrix.Euc_Dis(concept_Vectors[cluster[i]], i, normal_ConceptVectors[cluster[i]]);
				else
					for (i = 0; i < n_Clusters; i++)
						matrix.Euc_Dis(concept_Vectors[i], normal_ConceptVectors[i], sim_Mat, i); // problem
			}
			else
			...
			for (i = 0; i < col; i++)
			{
				cluster_quality[cluster[i]] += sim_Mat[(n_Clusters*i) + i];
			}
	...
	for (i = 0; i < col; i++)
	{
		...
		for (j = 0; j < n_Clusters; j++)
			...
			if (j != cluster[i])
			{
				...
				if (sim_Mat[(j*col) + i] < temp_sim) // sim_Mat[j][i]
				{
					temp_sim = sim_Mat[(j*col) + i]; // sim_Mat[j][i]
					temp_Cluster_ID = j;
				}
			}
		...
		if (temp_Cluster_ID != cluster[i])
		{
			...
			sim_Mat[(col*cluster[i]) + i] = temp_sim;
			changed++;
		}
	}
	return changed;
...
void Kmeans::Initialize_CV(Matrix matrix)
{
	...
	for (i = 0; i < col; i++)
	{
		cluster_quality[cluster[i]] += sim_Mat[(cluster[i]*col) +i]; // sim_Mat[cluster[i]][i]
	}
	//for (i = 0; i < n_Clusters; i++)
	//diff[i] = 0.0;
	...
	for (i = 1; i<n_Clusters; i++)
	{
		...
		for (j = 0; j<col; j++)
		{
			if (!mark[j])
			{
				cos_sum = 0.0;
				for (k = 0; k<i; k++)
					cos_sum += sim_Mat[(k*col) + j]; //sim_Mat[k][j];
				if (cos_sum > min)
				{
					min = cos_sum;
					min_ind = j;
